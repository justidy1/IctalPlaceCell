...
def gen_traj(maze,N_trials,p_correct,speed=20):
    ...
    trial_starts = np.linspace(0,max(t)-trial_length/speed,N_trials)
...
def firing_place_cell_linear_EXTENT(t,x,center_x,sigma=7):
    '''Gaussian where that has max = 1 to define the extent of the place field.
    ...
    return lambda t: (np.exp(-((x_t(t)-center_x)**2)/(2*sigma**2)))
...
def gen_spike_train(t_sim,spread=False,STDP_ictal=False,seed=1234):
    ...
    if STDP_ictal:
        ...
        # for each cell, keep the spike times in probability of the place field extent, i.e. thin the spike train
        for pf in pf_CA3:
            r = firing_place_cell_linear_EXTENT(t,x,pf,7) # 0.1 is the threshold for the extent
            t_in_extent = t[r(t) > 0.1]
            # keep the spike times in the extent of the place field, for each spike time,
            # check if it is in the extent of the place field
            s_times_rz_pf = []
            tol = 0.001
            for s in s_times_rz:
                if np.any(np.abs(s - t_in_extent) < tol):
                    s_times_rz_pf.append(s)
            CA3_placecell_spike_times.append(np.array(s_times_rz_pf))

        # create a reward zone ictal spike generator
        reward_zone_spikes = nest.Create("spike_train_injector",N_generators)

        # drive the reward zone generator
        for s_times_rz_pf,generator in zip(CA3_placecell_spike_times,reward_zone_spikes):
            nest.SetStatus(generator, {"spike_times": 1000*s_times_rz_pf+0.1,"allow_offgrid_times": True,"precise_times": False})

        ## connecting and weighting the rate by the proximity of the place field to the interictal spike
        nest.Connect(reward_zone_spikes, CA3_pyr[0:N_generators], syn_spec={'weight': 1.0,'receptor_type':0,'synapse_model':'bernoulli_synapse','p_transmit':1.0},
    ...
    return conn_CA3_to_CA3,conn_CA3_to_CA1,pf_CA3,pf_CA1,r_rz_right,r_rz_left,t,x,CA3_placecell_spike_times,spike_detector

# only run the code if this file is executed directly
if __name__ == '__main__':
    ...
    all_IS_r_left_ictal = []
    all_IS_r_right_ictal = []
    all_IS_r_left_ictal_spread = []
    all_IS_r_right_ictal_spread = []
    ...
    for i in range(N_reps):
        print(i)
        conn_CA3_to_CA3,conn_CA3_to_CA1,pf_CA3,pf_CA1,r_rz_right,r_rz_left,t,x,_,_ = gen_spike_train(t_sim,STDP_ictal=False,seed=i+1000)
        ...
        all_t_controls.append(t)
        all_x_controls.append(x)
        
        
        conn_CA3_to_CA3,conn_CA3_to_CA1,pf_CA3,pf_CA1,r_rz_right,r_rz_left,t,x,_,_ = gen_spike_train(t_sim,STDP_ictal=True,seed=i+1000)
        ...
        all_IS_r_left_ictal.append(r_rz_left)
        all_IS_r_right_ictal.append(r_rz_right)
        all_t_ictal.append(t)
        all_x_ictal.append(x)

        conn_CA3_to_CA3,conn_CA3_to_CA1,pf_CA3,pf_CA1,r_rz_right,r_rz_left,t,x,_,_ = gen_spike_train(t_sim,STDP_ictal=True,spread=True,seed=i+1000)
        ...
        all_IS_r_left_ictal_spread.append(r_rz_left)
        all_IS_r_right_ictal_spread.append(r_rz_right)
        all_t_ictal_spread.append(t)
        all_x_ictal_spread.append(x)

    np.savez_compressed('data/line_maze_data_spread_compressed_save_interictal_STIMPLACEFIELD_t300_1p0.npz',
            conn_CA3_to_CA3_controls=conn_CA3_to_CA3_controls,conn_CA3_to_CA1_controls=conn_CA3_to_CA1_controls,
            ...
            all_IS_r_left_ictal=all_IS_r_left_ictal,all_IS_r_right_ictal=all_IS_r_right_ictal,
            all_IS_r_left_ictal_spread=all_IS_r_left_ictal_spread,all_IS_r_right_ictal_spread=all_IS_r_right_ictal_spread,
            all_t_controls=all_t_controls,all_x_controls=all_x_controls,
            all_t_ictal=all_t_ictal,all_x_ictal=all_x_ictal,
            all_t_ictal_spread=all_t_ictal_spread,all_x_ictal_spread=all_x_ictal_spread)